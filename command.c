...
cmdInfo_t cmdInfo[] = {
    ...
    {PI_CMD_ADD, "ADD", 111, 0, 1},     {PI_CMD_AND, "AND", 111, 0, 1},     {PI_CMD_CALL, "CALL", 114, 0, 1}, {PI_CMD_CMDR, "CMDR", 111, 0, 1},
    {PI_CMD_CMDW, "CMDW", 111, 0, 1},   {PI_CMD_CMP, "CMP", 111, 0, 1},     {PI_CMD_DCR, "DCR", 113, 0, 1},   {PI_CMD_DCRA, "DCRA", 101, 0, 1},
    {PI_CMD_DIV, "DIV", 111, 0, 1},     {PI_CMD_EVTWT, "EVTWT", 111, 0, 1}, {PI_CMD_HALT, "HALT", 101, 0, 1}, {PI_CMD_INR, "INR", 113, 0, 1},
    {PI_CMD_INRA, "INRA", 101, 0, 1},   {PI_CMD_JM, "JM", 114, 0, 1},       {PI_CMD_JMP, "JMP", 114, 0, 1},   {PI_CMD_JNZ, "JNZ", 114, 0, 1},
    {PI_CMD_JP, "JP", 114, 0, 1},       {PI_CMD_JZ, "JZ", 114, 0, 1},       {PI_CMD_LD, "LD", 123, 0, 1},     {PI_CMD_LDA, "LDA", 111, 0, 1},
    {PI_CMD_LDAB, "LDAB", 111, 0, 1},   {PI_CMD_MLT, "MLT", 111, 0, 1},     {PI_CMD_MOD, "MOD", 111, 0, 1},   {PI_CMD_NOP, "NOP", 101, 0, 1},
    {PI_CMD_OR, "OR", 111, 0, 1},       {PI_CMD_POP, "POP", 113, 0, 1},     {PI_CMD_POPA, "POPA", 101, 0, 1}, {PI_CMD_PUSH, "PUSH", 113, 0, 1},
    {PI_CMD_PUSHA, "PUSHA", 101, 0, 1}, {PI_CMD_RET, "RET", 101, 0, 1},     {PI_CMD_RL, "RL", 123, 0, 1},     {PI_CMD_RLA, "RLA", 111, 0, 1},
    {PI_CMD_RR, "RR", 123, 0, 1},       {PI_CMD_RRA, "RRA", 111, 0, 1},     {PI_CMD_STA, "STA", 113, 0, 1},   {PI_CMD_STAB, "STAB", 111, 0, 1},
    {PI_CMD_SUB, "SUB", 111, 0, 1},     {PI_CMD_SYS, "SYS", 116, 0, 1},     {PI_CMD_TAG, "TAG", 114, 0, 1},   {PI_CMD_WAIT, "WAIT", 111, 0, 1},
    {PI_CMD_X, "X", 124, 0, 1},         {PI_CMD_XA, "XA", 113, 0, 1},       {PI_CMD_XOR, "XOR", 111, 0, 1},
...
static int
getNum(char* str, uintptr_t* val, int8_t* opt) {
  int f, n;
  intmax_t v;
...
int
cmdParse(char* buf, uintptr_t* p, unsigned ext_len, char* ext, cmdCtlParse_t* ctl) {
  ...
  if(ext_len < (4 * CMD_MAX_PARAM))
    return CMD_EXT_TOO_SMALL;
  ...
  switch(cmdInfo[idx].vt) {
    ...
    case 121: /* HC  FR  I2CRD  I2CRR  I2CRW  I2CWB I2CWQ  P
      ...
      if((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (ctl->opt[2] > 0) && ((int)p[2] >= 0))
        valid = 1;
    ...
    case 122: /* NB
      ...
      if((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (ctl->opt[2] > 0))
        valid = 1;
    ...
    case 123: /* LD  RL  RR
      ...
      if((ctl->opt[1] > 0) && (p[1] < PI_MAX_SCRIPT_VARS) && (ctl->opt[2] > 0))
        valid = 1;
    ...
    case 124: /* X
      ...
      if((ctl->opt[1] > 0) && (p[1] < PI_MAX_SCRIPT_VARS) && (ctl->opt[2] > 0) && (p[2] < PI_MAX_SCRIPT_VARS))
        valid = 1;
    ...
    case 131: /* BI2CO  HP  I2CO  I2CPC  I2CRI  I2CWB  I2CWW
      ...
      if((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (ctl->opt[2] > 0) && ((int)p[2] >= 0) && (ctl->opt[3] > 0) && ((int)tp1 >= 0)) {
        p[3] = 4;
        memcpy(ext, &tp1, 4);
        valid = 1;
      }
    ...
    case 133: /* FS
      ...
      if((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (ctl->opt[2] > 0) && (to1 == CMD_NUMERIC) && ((int)tp1 >= 0)) {
        p[3] = 4;
        memcpy(ext, &tp1, 4);
        valid = 1;
      }
    ...
    case 134: /* BSPIO
      ...
      if((ctl->opt[1] > 0) && ((int)p[1] >= 0) && (to1 == CMD_NUMERIC) && ((int)tp1 >= 0) && (to2 == CMD_NUMERIC) && ((int)tp2 >= 0) && (to3 == CMD_NUMERIC) &&
         ((int)tp3 >= 0) && (to4 == CMD_NUMERIC) && ((int)tp4 >= 0) && (to5 == CMD_NUMERIC)) {
        p[3] = 5 * 4;
        memcpy(ext + 0, &tp1, 4);
        memcpy(ext + 4, &tp2, 4);
        memcpy(ext + 8, &tp3, 4);
          ...
          if(to1 == CMD_NUMERIC) {
            if(((int)tp1 >= 0) && ((int)tp1 <= 255)) {
              ...
              *p8++ = tp1;
              ...
              break; /* invalid number, end of command */
          } else
            break;
        }
    ...
    case 194: /* I2CPK  I2CWI  I2CWK
      ...
      if((ctl->opt[1] == CMD_NUMERIC) && (ctl->opt[2] == CMD_NUMERIC) && ((int)p[1] >= 0) && ((int)p[2] >= 0) && ((int)p[2] <= 255)) {
        pars = 0;
        p8 = ext;
    ...
    case 196: /* WVAS
      ...
      if((ctl->opt[1] == CMD_NUMERIC) && ((int)p[1] >= 0) && (ctl->opt[2] == CMD_NUMERIC) && ((int)p[2] > 0) && (to1 == CMD_NUMERIC) && (to2 == CMD_NUMERIC) &&
         (to3 == CMD_NUMERIC)) {
        pars = 0;
...
int
cmdParseScript(char* script, cmdScript_t* s, int diags) {
  ...
  b = (sizeof(int) * (PI_MAX_SCRIPT_PARAMS + PI_MAX_SCRIPT_VARS)) + (sizeof(cmdInstr_t) * (len + 2) / 2) + len;
  ...
  while(ctl.eaten < len) {
    ...
    if((idx >= 0) || (idx != CMD_UNKNOWN_CMD)) {
      if(!cmdInfo[intCmdIdx].cvis)
        idx = CMD_NOT_IN_SCRIPT;
    }
  ...
  for(i = 0; i < s->instrs; i++) {
    ...
    if((instr.p[0] == PI_CMD_JMP) || (instr.p[0] == PI_CMD_CALL) || (instr.p[0] == PI_CMD_JZ) || (instr.p[0] == PI_CMD_JNZ) || (instr.p[0] == PI_CMD_JM) ||
       (instr.p[0] == PI_CMD_JP)) {
      resolved = 0;
