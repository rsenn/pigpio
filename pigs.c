...
initOpts(int argc, char* argv[]) {
  ...
  while((opt = getopt(argc, argv, "ax")) != -1) {
    switch(opt) {
      ...
      default: args++; report(PIGS_OPTION_ERR, "ERROR: bad option %c", optopt);
    }
  }
  return args;
}
...
void
print_result(int sock, int rv, cmdCmd_t cmd) {
  int i, r, ch;
  uint32_t* p;
  ...
  switch(rv) {
    ...
    case 5: printf("%s", cmdUsage); break;
    ...
    case 6: /*
      ...
      if(r < 0)
        report(PIGS_SCRIPT_ERR, "ERROR: %s", cmdErrStr(r));
      if(r > 0) {
        if(printFlags == PRINT_ASCII)
          printf(" ");

        for(i = 0; i < r; i++) {
          ch = response_buf[i];

          if(printFlags & PRINT_HEX)
            printf(" %hhx", ch);
    ...
    case 7: /* PROCP */
      if(r != (4 + (4 * PI_MAX_SCRIPT_PARAMS))) {
        ...
        printf("%d", p[0]);
        for(i = 0; i < PI_MAX_SCRIPT_PARAMS; i++) { printf(" %d", p[i + 1]); }
      }
      printf("\n");
      break;
...
void
get_extensions(int sock, int command, int res) {
  switch(command) {
    case PI_CMD_BI2CZ:
    case PI_CMD_BSCX:
    case PI_CMD_BSPIX:
...
main(int argc, char* argv[]) {
  ...
  for(i = args; i < argc; i++) {
    ...
    if(l < sizeof(command_buf)) {
      sprintf(command_buf + pp, "%s ", argv[i]);
      pp = l;
    }
  }
        ...
        if(command == PI_CMD_HELP) {
          ...
          if(sock != SOCKET_OPEN_FAILED) {
            if(send(sock, &cmd, sizeof(cmdCmd_t), 0) == sizeof(cmdCmd_t)) {
              if(p[3])
                send(sock, v, p[3], 0); /* send extensions */
              ...
              if(recv(sock, &cmd, sizeof(cmdCmd_t), MSG_WAITALL) == sizeof(cmdCmd_t)) {
                ...
                report(PIGS_CONNECT_ERR, "socket receive failed");
            } else
              report(PIGS_CONNECT_ERR, "socket send failed");
          } else
            report(PIGS_CONNECT_ERR, "socket connect failed");
        }
      } else
        report(PIGS_SCRIPT_ERR, "%s only allowed within a script", cmdInfo[idx].name);
    } else {
      if(idx == CMD_UNKNOWN_CMD)
        report(PIGS_SCRIPT_ERR, "%s? unknown command, pigs h for help", cmdStr());
      else
        report(PIGS_SCRIPT_ERR, "%s: bad parameter, pigs h for help", cmdStr());
    }
  }
